...
// ============ FUNÇÕES DAS PAYPAL (EMBUTIDAS DIRETAMENTE) ============

function check_paypal1($cc_data) {
    // PayPal Gate 1 - API direta
    $parts = explode('|', $cc_data);
    if (count($parts) != 4) {
        return "❌ DIE | {$cc_data} | FORMATO INVÁLIDO";
    }
    ...
    if (substr($card, 0, 1) == '4') {
        ...
        $cvv_len = 3;
    } elseif (substr($card, 0, 1) == '3') {
        $card_type = "AMEX";
        $cvv_len = 4;
    } else {
        return "❌ DIE | {$cc_data} | TIPO DE CARTÃO INVÁLIDO";
    }
    
    if (strlen($cvv) != $cvv_len) {
        return "❌ DIE | {$cc_data} | CVV INVÁLIDO";
    }
    
    // Simulação de resposta (substitua pela API real)
    // Para teste, cartão que termina com número par = LIVE
    $last_digit = intval(substr($card, -1));
    if ($last_digit % 2 == 0) {
        return "✅ LIVE | {$cc_data} | APROVADA | PayPal Gate 1";
    } else {
        return "❌ DIE | {$cc_data} | REPROVADA | PayPal Gate 1";
    }
}

function check_paypal2($cc_data) {
    // PayPal Gate 2 - API direta
    $parts = explode('|', $cc_data);
    if (count($parts) != 4) {
        return "❌ DIE | {$cc_data} | FORMATO INVÁLIDO";
    }
    ...
    $last_digit = intval(substr($card, -1));
    if ($last_digit % 3 == 0) {
        return "✅ LIVE | {$cc_data} | APROVADA | PayPal Gate 2";
    } else {
        return "❌ DIE | {$cc_data} | REPROVADA | PayPal Gate 2";
    }
}

// ============ PROCESSAR CHECKER VIA AJAX ============
if (isset($_GET['action']) && $_GET['action'] == 'check') {
    header('Content-Type: text/plain; charset=utf-8');
    $lista = $_GET['lista'] ?? '';
    $api = $_GET['api'] ?? 'paypal1';
    ...
    // Chamar a função correta baseada na API selecionada
    if ($api == 'paypal1') {
        echo check_paypal1($lista);
    } else {
        echo check_paypal2($lista);
    }
    exit;
}
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
                <div>
                    ...
                    <select id="api_selector">
                        <option value="paypal1">PAYPAL GATE 1</option>
                        <option value="paypal2">PAYPAL GATE 2</option>
                    </select>
                </div>
                <div>
                    <label>⚡ THREADS</label>
...
<script>
    ...
    async function processItem(line, api) {
        try {
            const response = await fetch(`?action=check&api=${api}&lista=${encodeURIComponent(line)}`);
            const result = await response.text();
            
            const isLive = result.includes('✅') || result.includes('LIVE') || result.includes('APROVADA');
